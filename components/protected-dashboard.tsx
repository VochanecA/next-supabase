...
// --- Component ---
export const ProtectedDashboard: React.FC<Props> = ({ user, subscriptions, transactions }) => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          <article className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-xl border border-gray-100 dark:border-gray-800">
            <header className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium px-3 py-1 rounded-full text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-900">User Profile</span>
            </header>
          ...
          <article className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-xl border border-gray-100 dark:border-gray-800">
            <header className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium px-3 py-1 rounded-full text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-900">Billing</span>
            </header>
              ...
              {subscriptions.length ? subscriptions.map(sub => (
                <div key={sub.subscription_id} className="pb-5 border-b border-gray-100 dark:border-gray-800 last:border-0 last:pb-0">
                  ...
                  <div className="text-sm text-gray-600 dark:text-gray-300 space-y-1 mt-3">
                    ...
                    {sub.next_billing_date && <p>Next billing: <span className="font-medium">{formatDate(sub.next_billing_date)}</span></p>}
                    {sub.trial_end_date && <p>Trial ends: <span className="font-medium text-orange-500 dark:text-orange-400">{formatDate(sub.trial_end_date)}</span></p>}
                  </div>
                </div>
              )) : (
          ...
          <article className="bg-white dark:bg-gray-900 rounded-2xl p-6 shadow-xl border border-gray-100 dark:border-gray-800">
            <header className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-medium px-3 py-1 rounded-full text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-900">History</span>
            </header>
            <div className="space-y-5 max-h-[400px] overflow-y-auto pr-2">
                  ...
                  <div className="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                    ...
                    {tx.card_network && tx.card_last_four && (
                      <p className="flex items-center">
                        ...
                        <span className="capitalize font-bold text-red-500">{tx.card_network}</span> card ending in <span className="font-bold ml-1">****{tx.card_last_four}</span>
                      </p>
                    )}
                  </div>
                </div>
        ...
        <section className="mt-8">
          <header className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-medium px-3 py-1 rounded-full text-blue-700 bg-blue-100 dark:text-blue-300 dark:bg-blue-900">Shortcuts</span>
          </header>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <button className="flex items-center justify-center px-4 py-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 rounded-xl text-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors shadow-sm">
              <CreditCardIcon className="w-5 h-5 mr-2" /> Update Billing
            </button>
            <button className="flex items-center justify-center px-4 py-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 rounded-xl text-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors shadow-sm">
              <ChevronRightIcon className="w-5 h-5 mr-2" /> Manage Plan
            </button>
          </div>
        </section>
      </main>
